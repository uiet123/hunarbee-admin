...
import { getResourceLibrary } from "@/lib/curriculum";
import type { ResourceLibraryItem, CurriculumResource } from "@/lib/curriculum/types";
import { generateId } from "@/lib/curriculum";

interface ResourceLibraryPickerProps {
  open: boolean;
  onClose: () => void;
  onSelect: (resource: CurriculumResource) => void;
}
...
export function ResourceLibraryPicker({ open, onClose, onSelect }: ResourceLibraryPickerProps) {
  ...
  const handleSelect = (item: ResourceLibraryItem) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl border border-navy/10 bg-surface-elevated shadow-2xl">
        <div className="flex items-center justify-between border-b border-navy/5 px-5 py-4">
          <h3 className="text-sm font-bold text-navy">Add from Resource Library</h3>
        ...
        <div className="max-h-[360px] overflow-y-auto portal-scrollbar px-3 py-2">
          {loading ? (
            ...
            <p className="py-8 text-center text-sm text-slate">No resources found.</p>
          ) : (
            filtered.map((item) => (
              <button
                key={item.id}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="rounded-full bg-navy/5 px-2 py-0.5 text-[10px] font-medium text-navy/60 shrink-0">
                        {TYPE_LABELS[item.type] || item.type}
                      </span>
                    </div>
                    <p className="text-xs text-slate line-clamp-1 mt-0.5">{item.description}</p>
